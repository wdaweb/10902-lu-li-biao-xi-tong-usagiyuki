...
const DBInfo = ["title", "class", "description", "sh" , "del", "id"];
const DBUid = "uid";
...
if (sizeof($_POST[DBInfo[0]]) == sizeof($_POST[DBInfo[1]]) && sizeof($_POST[DBInfo[1]]) == sizeof($_POST[DBInfo[2]]) &&
	...
	foreach($_POST[DBInfo[0]] as $idx => $val) {
		if (intval($_POST[DBInfo[4]][$idx]) == 1) {
			$sql = "delete from `experience` where `id` = :id && `uid` = :uid ";
			...
			$sql = "insert into `experience` (`title`, `class`, `description`, `uid`, `sh`) values(:title, :class, :description, :uid, :sh)";
			$stmt = $pdo->prepare($sql);
			$stmt->bindValue(':title', $_POST[DBInfo[0]][$idx], PDO::PARAM_STR);
			$stmt->bindValue(':class', $_POST[DBInfo[1]][$idx], PDO::PARAM_STR);
			$stmt->bindValue(':description', $_POST[DBInfo[2]][$idx], PDO::PARAM_STR);
			...
			$sql = "update `experience` set `title` = :title, `class` = :class, `description` = :description, `sh` = :sh where `id` = :id";
			$stmt = $pdo->prepare($sql);
			$stmt->bindValue(':title', $_POST[DBInfo[0]][$idx], PDO::PARAM_STR);
			$stmt->bindValue(':class', $_POST[DBInfo[1]][$idx], PDO::PARAM_STR);
			$stmt->bindValue(':description', $_POST[DBInfo[2]][$idx], PDO::PARAM_STR);
